...
function sum() {
  ...
  $("#calc .sum1").html(
    Number(sum1[0]).toLocaleString("ru") + ".<em>" + Number(sum1[1]) + "</em>"
  );
  $("#calc .sum2").html(
    Number(sum2[0]).toLocaleString("ru") + ".<em>" + Number(sum2[1]) + "</em>"
  );
...
var modal = document.getElementById("burger-menu"); // Get the modal
var closeModalLink = document.getElementById("home"); // Get the link to close the modal

closeModalLink.onclick = function () {
  // Attach an onclick event handler to the link
  modal.style.display = "none"; // Set the display property of the modal to "none" to hide it
};
